...
import { Phone, Clock, Shield, CheckCircle2, MapPin, Zap, ExternalLink } from 'lucide-react';
...
const WhatsAppIcon = ({ size = 24, fill = "currentColor", ...props }) => (
    <svg viewBox="0 0 24 24" width={size} height={size} fill={fill} xmlns="http://www.w3.org/2000/svg" {...props}>
...
/**
 * AutoKeysBasePage - Reusable base for city-specific auto keys landing pages.
 * All content is driven by i18n keys under `city_autokeys.<cityKey>.*`
 *
 * @param {string} props.cityKey - Key used to look up translations, e.g. "pilar"
 * @param {string} props.seoCanonical - Canonical URL for this page
 */
const AutoKeysBasePage = ({ cityKey, seoCanonical }) => {
    const { t } = useTranslation();
    const k = (key) => `city_autokeys.${cityKey}.${key}`;

    useEffect(() => {
        window.scrollTo(0, 0);
    }, []);

    const servicesList   = t(k('services_list'),   { returnObjects: true }) || [];
    const urgentFeatures = t(k('urgent_features'), { returnObjects: true }) || [];
    const zonesList      = t(k('zones_list'),      { returnObjects: true }) || [];
    const pricingList    = t(k('pricing_list'),    { returnObjects: true }) || [];
    const partnerItems   = t(k('partner_items'),   { returnObjects: true }) || [];
    const hasExtra       = !!t(k('extra_title'), { defaultValue: '' });

    return (
        <div className="ak-page">
            <SEO
                title={t(k('seo_title'))}
                description={t(k('seo_description'))}
                keywords={t(k('seo_keywords'))}
            ...
            <section
                className="ak-hero"
                style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.9)), url(${heroBg})` }}
            >
                <div className="container ak-hero-container">
                    ...
                    >
                        <h1 className="ak-hero-title">{t(k('hero_title'))}</h1>
                        <p className="ak-hero-subtitle">{t(k('hero_subtitle'))}</p>
                        <motion.div
                ...
                <div className="container">
                    <div className="ak-services-grid">
                        <div className="ak-services-main">
                            <div className="ak-header-highlight">{t(k('services_title'))}</div>
                            <h2 className="ak-section-title">{t(k('services_title'))}</h2>
                            <p className="ak-section-subtitle">{t(k('services_desc'))}</p>
                        ...
                        <div className="ak-services-side">
                            ...
                            <p className="ak-side-text">{t(k('services_footer'))}</p>
            ...
            <section className="ak-section ak-bg-black ak-urgent-section">
                <div className="container">
                    <div className="ak-section-header">
                        <h2 className="ak-section-title ak-text-white">{t(k('urgent_title'))}</h2>
                        <p className="ak-section-subtitle ak-text-light">{t(k('urgent_text'))}</p>
                    </div>
            ...
            {/* Optional extra section (e.g. international residents) */}
            {hasExtra && (
                <section className="ak-section ak-bg-white">
                    <div className="container">
                        <div className="ak-section-header">
                            <h2 className="ak-section-title">{t(k('extra_title'))}</h2>
                            <p className="ak-section-subtitle">{t(k('extra_text'))}</p>
                        </div>
                    </div>
                </section>
            )}
            ...
            <section className="ak-section ak-bg-gray">
                <div className="container">
                    <div className="ak-header-highlight">{t(k('zones_title'))}</div>
                    <div className="ak-zones-header">
                        <h2 className="ak-section-title ak-max-w-md">{t(k('zones_text'))}</h2>
                        <span className="ak-zones-footer">{t(k('zones_footer'))}</span>
                    </div>
                    <div className="ak-pill-grid">
                        {zonesList.map((item, idx) => {
                            const autoKeyPath = getAutoKeyPath(item);
                            ...
                            return autoKeyPath ? (
                                ...
                                <div key={idx} className="ak-pill ak-pill-white ak-hover-border">
                                    <MapPin size={20} className="ak-icon-yellow" />
                                    {item}
                                </div>
                            );
            ...
            <section className="ak-section ak-bg-white">
                <div className="container">
                    <div className="ak-section-header">
                        <h2 className="ak-section-title">{t(k('pricing_title'))}</h2>
                        <p className="ak-section-subtitle">{t(k('pricing_text'))}</p>
                    </div>
                ...
                <div className="container">
                    <div className="ak-partner-banner">
                        ...
                        <div className="ak-partner-content">
                            <div className="ak-partner-tag">{t(k('partner_tag'))}</div>
                            <h3 className="ak-partner-title">{t(k('partner_title'))}</h3>
                        ...
                        <a
                            href="https://surestecerrajeros.es"
                            target="_blank"
            ...
            <section className="ak-cta-section">
                ...
                <div className="container ak-cta-container">
                    <h2 className="ak-cta-title">{t(k('cta_title'))}</h2>
                    <p className="ak-cta-subtitle">{t(k('cta_text'))}</p>
...
export default AutoKeysBasePage;
